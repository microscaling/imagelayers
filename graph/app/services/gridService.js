    ...
    function() {
      ...
      var buildMatrix = function(images) {
        ...
        for (var i=0; i < sorted_images.length; i++) {
          for (var j=0; j < sorted_images[i].layers.length; j++) {
            var layer = sorted_images[i].layers[j];
           if (inventory[layer.id] !== undefined) {
              j = changeInventoryCounts(sorted_images, layer, i);
            } else {
              inventory[layer.id] = { 'image': sorted_images[i],  'row': j, 'count': 1 };
